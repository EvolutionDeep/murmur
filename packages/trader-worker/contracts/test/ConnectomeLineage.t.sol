...
contract ConnectomeLineageTest is Test {
    ...
    function _genesis(bytes32 h) internal {
        vm.prank(committer);
        reg.commit(h, bytes32(0), bytes32(0), reg.OP_GENESIS(), 0, breeder);
    }
    ...
    function testMutateLineageIntegrity() public {
        _genesis(GEN_A);
        vm.prank(committer);
        reg.commit(CHILD_MUT, GEN_A, bytes32(0), reg.OP_MUTATE(), 1, breeder);
    ...
    function testCrossLineageUsesMaxParentGenPlusOne() public {
        ...
        reg.commit(CHILD_MUT, GEN_A, bytes32(0), reg.OP_MUTATE(), 1, breeder);
        vm.prank(committer);
        reg.commit(CHILD_CROSS, CHILD_MUT, GEN_B, reg.OP_CROSS(), 2, breeder);
    ...
    function testRevertNotCommitter() public {
        ...
        reg.commit(GEN_A, bytes32(0), bytes32(0), reg.OP_GENESIS(), 0, breeder);
    }
    ...
    function testRevertSkippedGeneration() public {
        ...
        reg.commit(CHILD_MUT, GEN_A, bytes32(0), reg.OP_MUTATE(), 5, breeder); // must be gen 1
    }

    function testRevertBadGenesis() public {
        vm.prank(committer);
        vm.expectRevert(ConnectomeLineage.BadGenesis.selector);
        reg.commit(GEN_A, GEN_B, bytes32(0), reg.OP_GENESIS(), 0, breeder); // genesis must have no parents
    }
    ...
    function testRevertCrossMissingSecondParent() public {
        ...
        reg.commit(CHILD_CROSS, GEN_A, bytes32(0), reg.OP_CROSS(), 1, breeder); // cross needs two parents
    }

    function testRevertZeroBreeder() public {
        vm.prank(committer);
        vm.expectRevert(ConnectomeLineage.ZeroBreeder.selector);
        reg.commit(GEN_A, bytes32(0), bytes32(0), reg.OP_GENESIS(), 0, address(0));
    }
    ...
    function testFuzzGenerationContiguity(uint32 genA, uint32 genB) public {
        ...
        vm.prank(committer);
        reg.commit(child, a, b, reg.OP_CROSS(), expect, breeder);
    ...
    function _chain(bytes32 leaf, uint32 gen) internal {
        ...
        for (uint32 g = 0; g <= gen; g++) {
            ...
            vm.prank(committer);
            if (g == 0) reg.commit(h, bytes32(0), bytes32(0), reg.OP_GENESIS(), 0, breeder);
            else reg.commit(h, prev, bytes32(0), reg.OP_MUTATE(), g, breeder);
            prev = h;
        }
    }
}
