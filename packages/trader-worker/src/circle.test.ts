...
test("includes the paid resource only when a URL is supplied (internal deals omit it)", () => {
  const withRes = buildCircleSettleBody(
    settleArgs({ resourceUrl: "https://api.muros.live/signal/pulse", resourceDescription: "arc pulse", resourceMime: "application/json" }),
  ).body;
  assert.deepEqual(withRes.paymentPayload.resource, {
    url: "https://api.muros.live/signal/pulse", description: "arc pulse", mimeType: "application/json",
  });
